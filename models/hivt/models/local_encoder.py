...
class LocalEncoder(nn.Module):
    ...
    def forward(self, data: TemporalData, pose_embed: torch.Tensor) -> torch.Tensor:
        ...
        if self.parallel:
            ...
            for t in range(self.historical_steps):
                edge_index, edge_attr = self.drop_edge(data[f'edge_index_{t}'], data[f'edge_attr_{t}'])
                out[t] = self.aa_encoder(x=data.x[:, t], pose=pose_embed, t=t, edge_index=edge_index, edge_attr=edge_attr,
