...
sys.path.append('/root/Demo/mmdet3d')
import mmcv
import numpy as np
import pandas as pd
...
class AV2MonoDataset(Dataset):
    ...
    def get_ann_info(self, index):
        ...
        """
        breakpoint()
        info = self.data_infos[index]['annotations']
        img_info = self.data_infos[index]["images"]
    ...
    def crop_bbox(self, out_dir, show=False, pipeline=None):
        """Results visualization.
...
if __name__ == '__main__':
    ...
    config = {
        'data_root': 'av2_data/sensor/',
        'ann_file': 'av2_data/sensor/{}/av2_infos_train_mono3d.coco.json'.format(VERSION),
        'classes': class_names,
    ...
    config = [
        './AV2_DATASET_ROOT',
        './AV2_DATASET_ROOT/{}/av2_infos_train.pkl'.format(VERSION),
        eval_pipeline,
        class_names,
        input_modality,
        'Lidar'
    ]
    dataset = AV2MonoDataset(*config)
    dataset.crop_bbox(out_dir='AV2_DATASET_ROOT', show=True, pipeline=eval_pipeline)
    breakpoint()